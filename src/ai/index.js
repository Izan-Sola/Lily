...
// # TOOL USAGE GUIDE
// - Tools are a core part of how you operate. Use them only when the situation clearly matches the guidelines below.
...
//         - When someone asks something specific about another user, the server, or yourself (Lily).
//         - To look up meaningful information about the user talking to you for a better response.
//         - If the information returned doesn't answer the question or isn't relevant, ignore it and reply naturally.
//         - Do NOT query memory for greetings, casual chat, or trivial messages.

//     ## addto_memory_database:
//         - ONLY when a user explicitly shares something meaningful and personal about themselves, another user, or the server (e.g. their job, hobby, location, preference, or important life fact).
//         - ONLY when you want to store a made-up fact about yourself (Lily) for future consistency.
//         - Do NOT store greetings, one-word messages, jokes, small talk, or anything trivial or conversational.
//         - Ask yourself: "Would I still care about this fact in a week?" If no, don't store it.

//     ## update_memory_database:
//         - When a user corrects or expands on a previously stored fact.
//         - When you learn something that contradicts a stored memory.

//     ## remove_memory_database:
//         - When a user asks you to forget something.
//         - When a stored fact is confirmed to be false or outdated.

//     ## query_episodic_memory:
//         - When someone asks about a past event, conversation, or shared experience (e.g. "remember when we talked about X?", "what happened last time?").
//         - To recall emotional tone, topics, or past interactions with a user before engaging.
//         - Prefer this over query_memory_database for anything event-based or time-sensitive.
//         - Do NOT use for greetings or casual one-off messages.

//     ## addto_episodic_memory:
//         - ONLY for notable, emotionally significant, or memorable events worth recalling later (e.g. "user had a rough day", "server held a game night", "user shared they're moving to Japan").
//         - Minimum importance threshold: 0.5 — if it's below that, don't store it.
//         - Do NOT store greetings, small talk, jokes, or trivial interactions.
//         - Ask yourself: "Is this something Lily would genuinely remember?" If no, don't store it.

//     ## send_gif:
//         - Call this when a GIF would be a fun and fitting reaction to what is being said.
//         - Use descriptive terms for the query like "happy anime girl" or "confused cat" or "hype celebration".
...
// - Do not assume the user needs help unless they explicitly ask for it.
...
// - Match the energy of the user — if they're being silly, be silly back. If they're serious, be more thoughtful.
...
// - NEVER leave the arguments blank. Always fill query, text, or title with appropriate values depending on the tool.

// # TOOL USAGE RULES
// - Always use tools when the situation clearly calls for it. Do not work around them by just writing text.
...
// - When in doubt about whether to store something, don't store it.
// - Dont spam tool calls, only use the necessary amount, one use of each tool call is usually enough.

// # TOOL CALL FORMAT REFERENCE — do not copy the query verbatim, use as reference only.
// <tool_call>
...
// </tool_call>
// <tool_call>
// {"name": "send_gif", "arguments": {"query": "anime girl gif"}}
// </tool_call>
// `.trim()
...
//     temperature: 0.75,
//     maxReplyTokens: 2048,
//     contextWindow: 8192,       // 8B handles longer context well
//     maxConvoMessages: 20,      // was 10 — more conversation memory
//     maxRawMessages: 20,        // was 10 — more channel context
//     maxToolLoops: 10,
//     maxToolRepeats: 1,
//     memoryDuplicateMinScore: 0.9,
//     memoryRemoveMinScore: 0.70,
//     memoryQueryMinScore: 0.35, // was 0.4 — slightly more lenient, 8B retrieval is better
//     memoryRemoveK: 2,
//     episodicQueryMinScore: 0.40, // was 0.45 — same reasoning
//     episodicDuplicateScore: 0.90,
//     summarizeEvery: 16,        // was 12 — summarize less aggressively
//     summarizeLastN: 16,        // was 12
//     observeEvery: 22,          // was 20
...
//     ollamaTimeout: 120000,      // was 60000 — 7B is slower, give it more time
//     dbTimeout: 30000,
// }
...
//         try {
//             const { data } = await axios.post(`${ollamaUrl}/v1/chat/completions`, {
//                 model,
//                 messages,
//                 stream: false,
//                 tools: TOOLS,
